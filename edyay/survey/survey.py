...
class Survey:
    def create_alias_file(self, filename_override, reset=False):
        ...
        if Path(self.alias_filename).is_file() and not reset:
            logging.info(
                f"Alias file found at {self.alias_filename}... using that. "
    ...
    def create_exclude_file(self, filename_override):
        ...
        if Path(self.exclude_filename).is_file():
            logging.info(
                f"Exclude file found at {self.exclude_filename}... using that."
            )
    ...
    def update_exclusions(self):
        ...
        exclusion_mask = pd.Series(
            [False] * len(self.data_df.index), index=self.data_df.index
        )
        ...
        if n_excluded > 0:
            logging.info(
                f"{n_excluded} responses dropped based on specified exclusions"
            )
        else:
    ...
    def plot_question(self, question_str, title=None, **kwargs):
        ...
        if question_str in self.multiselect_questions:
            ...
            if "Response" in sub_df.columns:
                plot_df = sub_df[
                    ~(sub_df.Response == "") & (sub_df.Response.notnull())
        ...
        for question in questions:
            ...
            try:
                ...
                if isinstance(result, str):
                    ...
                    df, fig = result
                    fig.savefig(
                        f"{path}{self.full2alias[question]}.png",
                        dpi=300,
                        bbox_inches="tight",
                    )
